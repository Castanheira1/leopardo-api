...
function sondarControles() {
  ...
  const doentes = [];
  for (const el of document.querySelectorAll("button, a[href], input, select, .nav-menu-item")) {
  ...
  try {
    for (const cen of CENARIOS) {
      for (const t of TAMANHOS) {
        ...
        ok(menuAbriu, `${tag}: hambúrguer abre o menu`);

        if (menuAbriu) {
          // Menu simples (absolute abaixo do botão): não pode cobrir o ☰.
          ...
          ok(cobertura.livre && cobertura.abaixo && cobertura.visivel,
            `${tag}: menu abre abaixo do hambúrguer e fica visível`);

          let chamou = false;
          let painelVisivel = false;
          try {
            await page.locator("#navMenu .nav-menu-item", { hasText: /^Locais$/ }).tap({ timeout: 4000 });
